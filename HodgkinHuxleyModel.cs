...
public class HodgkinHuxleyModel : MonoBehaviour
{
    ...
    float input_current = 60f;

    bool isFiring = false;
    bool canFire = true;
    float refrac = 0;
    float refracLength = 0.5f;
    HHModelCurrentSyn otherNeuronScript;
    ...
    void Start()
    {
        ...
        otherNeuronScript = GameObject.Find("HH-Neuron 2").GetComponent<HHModelCurrentSyn>();
    }
    ...
    void FixedUpdate()
    {
        ...
        if(states[0] > 0 && canFire)
        {
            canFire = false;
            isFiring = true;
            otherNeuronScript.isInput = 1f;
        }
        if(isFiring) {
            isFiring = false;
        }
    ...
    void Update()
    {
        ...
        if(!canFire)
        {
            refrac += Time.deltaTime;

            if(refrac > refracLength) {
                canFire = true;
                refrac = 0;
            }
        }
